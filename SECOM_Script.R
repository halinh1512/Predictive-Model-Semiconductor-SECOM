...
# !!! parameter an be defined at beginning of script, use more appropreate name / not self-explaining
...
# !!! good approach but histogram should be added
...
# Looking at the percentage of "NaN" per column, we determined our threshold to __20%__ (red line). In other words, any column that has greater than 20% of "NaN" shall be removed.
...
# We considered whether we could remove the entire record or not. Hence we looked at the at the percentage of "NaN" per row.
...
# Note: Features with desired outcome (i.e. Fails or "1") were not removed from the data set. This was factored into our calculation of the threshold mentioned above. 
# We determined that the percentage of NA per row would be too low to exclude entire records
# Therefore, we transform the outlier to NA and will impute values to replace outliers in the following stages.

# Transform outliers to NA
secom.trimmed<-secom.clean.1
secom.trimmed[abs(secom.zscore)>3] = NA

# !!! perfect usage of apply
# !!! But more appropreate approach would be to focus on vola and define a threshold.

## 4. Features with low volatility


ncol(secom[,apply(secom.data,2,var,na.rm=TRUE) == 0])
...
# Histogram of volatility (The range of absolute coefficient of variance is large but we focus on xlim from 0 to 500, because higher volatility columns will be kept anyways)
par(las=1)
...
# we decide the volatility threshold to be 5 and will remove the columns with volatility less than threshold
...
# We try Min-Max scaling with the range of [0,1] and also z-score scaling (standardization) and select Min-Max scaling for next steps
# Min-Max scaling of range [0,1]
secom.scaled<-data.frame(apply(secom.clean.2,2,rescale,to=c(0,1)))

# z-score scaling (standardization)
# secom.zscore<-data.frame(scale(secom.clean.2))

# Algorithms with assumption of normal distribution will require data transformation before building the model. 
# Hence, we also prepare a Yeo-Johnson transformation function for later step (because SECOM contains zero and negative values).
...
# We performed PCA as a method for feature selection/reduction. 
...
# Therefore, we need to implement a certain methods to balance the data in order to build an efficient model.
# ROSE
...
# Predict on Test set
## Using model built on ROSE
#secom.model_predict.rose<-predict(secom.model_rf.rose,secom.test.model.rescaled,type="prob")
## data frame of actual Status and predicted Status
#secom.final.rose<-data.frame(actual = secom.test.model.rescaled$Status%>%as.factor(), predict = secom.model_predict.rose)
## probability threshold = 0.5
#secom.final.rose$predict<-ifelse(secom.final.rose$predict.1>0.5, 1, -1)%>%as.factor()
